...
In this section, we will show you how to bind your `BrowserWindow` or `child process` instance to
a `Broker` object. Which on the `main` process works as a bridge between all of the application processes
that you decide to bind to it, besides that function It can also handle and send Its messages.
It's a **crucial** part of the library, so make sure you set it up and connect your processes to it.
...
function bootstrap() {
  ...
  broker = BrokerFactory.createMainBroker({
...
config object. Let's assume that we already have a pre-bundled .js file in `./webpack/child.js` directory
